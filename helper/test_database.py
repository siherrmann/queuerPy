...
class PostgresTestMixin:
    """
    Mixin class for test cases that need vanilla PostgreSQL database containers.
    Uses PostgreSQL without TimescaleDB extension.
    """

    @classmethod
    def setup_class(cls):
        """Set up vanilla PostgreSQL container for the entire test class (lazy initialization)."""
        # Only create container if we don't already have one for this class
        if not hasattr(cls, "_container_initialized"):
            cls.container: PostgresContainer = PostgresContainer(
                "postgres:latest",
            ...
            # Create database configuration with TimescaleDB disabled
            cls.db_config = DatabaseConfiguration(
                host=cls.container.get_container_host_ip(),
                port=cls.container.get_exposed_port(5432),
                database="test_db",
