...
from .error import database_error, QueuerError
...
class DatabaseConfiguration:
    ...
    def from_env(cls) -> 'DatabaseConfiguration':
        ...
        with_table_drop = os.getenv("QUEUER_DB_WITH_TABLE_DROP", "false").lower() == "true"
        
        # Validate required fields
        if not all([host.strip(), str(port), database.strip(), username.strip(), schema.strip()]):
...
class Database:
    ...
    def __init__(self, name: str, config: Optional[DatabaseConfiguration] = None, 
                 logger: Optional[logging.Logger] = None, auto_connect: bool = True):
    ...
    def connect_to_database(self) -> None:
        ...
        """
        if not self.config:
            raise database_error("Database configuration is required for connection")
        ...
        try:
            ...
            try:
                self.instance.execute("CREATE EXTENSION IF NOT EXISTS pg_trgm;")
            ...
            raise database_error("Failed to connect to database", e)
    ...
    def check_table_existence(self, table_name: str) -> bool:
        ...
        if not self.instance:
            raise database_error("Database connection not established")
        ...
        try:
            ...
            raise database_error(f"Failed to check table existence for {table_name}", e)
    ...
    def create_index(self, table_name: str, column_name: str) -> None:
        ...
        try:
            ...
            self.instance.rollback()
            raise database_error(f"Failed to create index on {table_name}.{column_name}", e)
    ...
    def create_combined_index(self, table_name: str, column_name1: str, column_name2: str) -> None:
        """
        ...
        try:
            ...
            self.instance.rollback()
            raise database_error(f"Failed to create combined index on {table_name}.{column_name1},{column_name2}", e)
    
    def create_unique_combined_index(self, table_name: str, column_name1: str, column_name2: str) -> None:
        """
        ...
        try:
            ...
            self.instance.rollback()
            raise database_error(f"Failed to create unique combined index on {table_name}.{column_name1},{column_name2}", e)
    ...
    def drop_index(self, table_name: str, json_map_key: str) -> None:
        ...
        try:
            ...
            self.instance.rollback()
            raise database_error(f"Failed to drop index {index_name}", e)
    ...
    def health(self) -> Dict[str, str]:
        ...
        try:
            ...
            stats["transaction_status"] = status_map.get(info.transaction_status, "unknown")
    ...
    def create_table_if_not_exists(self, table_name: str, create_sql: str) -> bool:
        ...
        try:
            ...
            self.instance.rollback()
            raise database_error(f"Failed to create table {table_name}", e)
    ...
    def drop_table_if_exists(self, table_name: str) -> bool:
        ...
        try:
            ...
            self.instance.rollback()
            raise database_error(f"Failed to drop table {table_name}", e)
    ...
    def execute_sql_file(self, file_path: str) -> None:
        ...
        try:
            ...
            self.instance.rollback()
            raise database_error(f"Failed to execute SQL file {file_path}", e)
...
def new_database(name: str, config: DatabaseConfiguration, 
                logger: Optional[logging.Logger] = None, auto_connect: bool = True) -> Database:
    """
...
def new_database_from_env(name: str = "queuer", 
                         logger: Optional[logging.Logger] = None,
                         auto_connect: bool = False) -> Database:
    """
...
def new_database_with_connection(name: str, connection: Connection,
                                logger: Optional[logging.Logger] = None) -> Database:
    """
