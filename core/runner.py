...
class Runner:
    ...
    def __init__(self, task: Callable, parameters: Optional[List[Any]] = None, options: Optional[List[OnError]] = None):
    ...
    def _validate_parameters(self):
        ...
        try:
            ...
            if len(self.parameters) != param_count:
                raise ValueError(f"Task expects {param_count} parameters, got {len(self.parameters)}")
        ...
        async def _async_execute():
            try:
                ...
                if asyncio.iscoroutinefunction(self.task):
                    ...
                    result = await loop.run_in_executor(None, self.task, *self.parameters)
    ...
    def get_results(self, timeout: Optional[float] = None) -> Any:
        ...
        try:
            if self._loop and not self._loop.is_closed():
                ...
                future = asyncio.run_coroutine_threadsafe(_get_results_async(), self._loop)
                if timeout is None:
