...
class TestRetryer:
    ...
    async def test_successful_execution(self):
        ...
        mock_function = Mock()
        options = OnError(
            max_retries=3, retry_delay=0.1, retry_backoff=RetryBackoff.NONE
        )

        retryer = Retryer(mock_function, options)
        result = await retryer.retry()
    ...
    async def test_retry_with_eventual_success(self):
        ...
        options = OnError(
            max_retries=5, retry_delay=0.01, retry_backoff=RetryBackoff.NONE
        )
    ...
    @pytest.mark.asyncio
    async def test_retry_with_permanent_failure(self):
        """Test retry mechanism with permanent failure."""
        mock_function = Mock(side_effect=Exception("Persistent error"))
        options = OnError(
            max_retries=3, retry_delay=0.01, retry_backoff=RetryBackoff.NONE
        )
    ...
    async def test_linear_backoff(self):
        ...
        options = OnError(
            max_retries=3, retry_delay=0.1, retry_backoff=RetryBackoff.LINEAR
        )
        retryer = Retryer(failing_function, options)
    ...
    async def test_exponential_backoff(self):
        ...
        options = OnError(
            max_retries=3, retry_delay=0.05, retry_backoff=RetryBackoff.EXPONENTIAL
        )
        retryer = Retryer(failing_function, options)
    ...
    def test_invalid_options(self):
        ...
        invalid_options = OnError(
            max_retries=0, retry_delay=0.1, retry_backoff=RetryBackoff.NONE
        )
        with pytest.raises(ValueError, match="No valid retry options provided"):
