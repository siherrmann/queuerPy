...
def new_listener(broadcaster) -> 'Listener[T]':
    """Create a new listener. Mirrors Go's NewListener function."""
    return Listener[T](broadcaster)
...
class Listener(Generic[T]):
    ...
    def listen(self, stop_event: threading.Event, ready_event: Optional[threading.Event], 
