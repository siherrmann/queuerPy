    ...
    def test_new_worker(self):
        ...
        for test_case in test_cases:
            with self.subTest(name=test_case["name"]):
                try:
                    worker = new_worker(test_case["worker_name"], test_case["max_concurrency"])
                    
                    if test_case["want_err"]:
                        self.fail(f"NewWorker should return an error for {test_case['name']}")
                    else:
                        self.assertIsNotNone(worker, "Worker should not be None for valid options")
                        self.assertEqual(test_case["worker_name"], worker.name, "Worker name should match the provided name")
                        self.assertEqual(test_case["max_concurrency"], worker.max_concurrency, "Worker max concurrency should match the provided value")
                        self.assertEqual(WorkerStatus.READY, worker.status, "Worker status should be READY by default")
                        
                except Exception as e:
                    if not test_case["want_err"]:
                        self.fail(f"NewWorker should not return an error for {test_case['name']}: {e}")
        ...
        for test_case in test_cases:
            with self.subTest(name=test_case["name"]):
                try:
                    ...
                    if test_case["want_err"]:
                        self.fail(f"NewWorkerWithOptions should return an error for {test_case['name']}")
                    else:
                        self.assertIsNotNone(worker, "Worker should not be None for valid options")
                        self.assertEqual(test_case["worker_name"], worker.name, "Worker name should match the provided name")
                        self.assertEqual(test_case["max_concurrency"], worker.max_concurrency, "Worker max concurrency should match the provided value")
                        self.assertEqual(WorkerStatus.READY, worker.status, "Worker status should be READY by default")
                        self.assertEqual(test_case["options"], worker.options, "Worker options should match the provided options")
                        
                except Exception as e:
                    if not test_case["want_err"]:
                        self.fail(f"NewWorkerWithOptions should not return an error for {test_case['name']}: {e}")
