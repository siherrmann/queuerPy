            ...
            with self.subTest(name=test_case["name"]):
                try:
                    ...
                    if test_case["want_err"]:
                        self.fail(f"NewTask should return an error for {test_case['name']}")
                    else:
                        self.assertIsNotNone(task, "Task should not be None for valid options")
                        self.assertEqual(test_case["exp_name"], task.name, "Task name should match the expected name")
                        
                        # Get function signature for parameter validation
                        sig = inspect.signature(test_case["task"])
                        actual_in_params = len(sig.parameters)
                        self.assertEqual(test_case["exp_in_params"], actual_in_params, "Input parameters count should match expected")
                        
                        # For output parameters, check return annotation
                        return_annotation = sig.return_annotation
                        if return_annotation != inspect.Signature.empty:
                            # Has return type annotation
                            self.assertGreaterEqual(test_case["exp_out_params"], 0, "Should have output parameters when return annotation is present")
                        
                except Exception as e:
                    if not test_case["want_err"]:
                        self.fail(f"NewTask should not return an error for {test_case['name']}: {e}")
        ...
        for test_case in test_cases:
            with self.subTest(name=test_case["name"]):
                try:
                    ...
                    if test_case["want_err"]:
                        self.fail(f"NewTaskWithName should return an error for {test_case['name']}")
                    else:
                        self.assertIsNotNone(task, "Task should not be None for valid options")
                        self.assertEqual(test_case["task_name"], task.name, "Task name should match the provided name")
                        
                except Exception as e:
                    if not test_case["want_err"]:
                        self.fail(f"NewTaskWithName should not return an error for {test_case['name']}: {e}")
        ...
        for callable_obj in [regular_function, lambda_func, callable_instance]:
            with self.subTest(callable_type=type(callable_obj).__name__):
                try:
                    ...
                    self.fail(f"Should accept callable of type {type(callable_obj).__name__}: {e}")
