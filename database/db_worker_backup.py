...
class WorkerDBHandler:
    ...
    def create_table(self) -> None:
        ...
        with self.db.instance.cursor() as cur:
            cur.execute("""
                ...
                CREATE UNIQUE INDEX IF NOT EXISTS worker_rid_idx ON worker(rid);
    ...
    def update_stale_workers(self, stale_threshold: timedelta = timedelta(minutes=5)) -> int:
        """
    ...
    def _row_to_worker(self, row: Dict[str, Any]) -> Worker:
        ...
        if row.get('output_rid'):
            worker.rid = row['output_rid'] if isinstance(row['output_rid'], UUID) else UUID(row['output_rid'])
        ...
        worker.available_next_interval_funcs = row.get('output_available_next_interval', []) or []
        ...
        if row.get('output_options'):
            ...
            options_data = json.loads(row['output_options']) if isinstance(row['output_options'], str) else row['output_options']
